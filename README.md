...
| 🔍 **快速搜索** | 内置站内模糊搜索（关键词高亮），迅速定位所需网站 |
| 📂 **分类清晰** | 通过多级分类组织书签，浏览直观高效 |
| ⭐ **常用收藏** | 卡片一键标星，本地保存常用书签，支持「只看常用」过滤 |
| ⌨️ **键盘快捷键** | `/` 或 `Ctrl/⌘+K` 聚焦搜索，`Esc` 一键清空 |
| 🌗 **主题跟随系统** | 首次访问自动跟随系统深浅色偏好，手动切换后以手动为准 |
| 📲 **PWA 支持** | 可安装到桌面/手机主屏，静态资源离线缓存秒开 |
| 🔒 **安全后台** | 基于 KV 的管理员认证，提供完整的书签增删改查后台 |
| 📝 **用户提交** | 支持访客提交书签，经管理员审核后显示（可通过环境变量关闭） |
| ⚡ **性能卓越** | 利用 Cloudflare 边缘缓存，秒级加载，节省 D1 数据库读取成本 |
| 🤖 **SEO 友好** | 动态 robots.txt / sitemap.xml、JSON-LD 结构化数据 |
...
- ⭐ **常用收藏**：卡片悬停即可一键标星，星标书签本地保存（localStorage），支持「只看常用」过滤与琥珀色高亮标识，跨设备登录同一浏览器也能保留。
- ⌨️ **键盘快捷键**：`/` 或 `Ctrl/⌘+K` 快速聚焦搜索框，`Esc` 一键清空搜索或关闭弹窗，键盘党效率拉满。
- 🔍 **搜索关键词高亮**：站内搜索时命中关键词以琥珀色 mark 标记，结果一目了然。
- 🌗 **主题跟随系统**：首次访问自动跟随系统深浅色偏好（prefers-color-scheme），手动切换后仍以手动选择为准。
- 📲 **PWA 支持**：动态 manifest（站点名随后台设置）+ Service Worker 静态资源离线缓存 + 可安装图标，导航站可"装"进系统。
- 🤖 **SEO 增强**：新增动态 `robots.txt`、`sitemap.xml`（含全部分类页）与 JSON-LD 结构化数据，收录更友好。
...
<!-- changelog:start -->
- 🔧 **2026-08-28**：常用收藏、键盘快捷键、PWA、SEO 与多处体验优化
- ⚡ **2026-08-14**：优化缓存策略并提升加载性能
- 🐞 **2026-05-06**：修复若干问题并提升稳定性
- 🐞 **2026-05-05**：修复若干问题并提升稳定性
- 🛡️ **2026-04-20**：优化登录会话与安全防护
- 🎨 **2026-04-19**：优化卡片样式与后台界面体验
- 🎨 **2026-04-17**：优化卡片样式与后台界面体验，并提升页面性能
- ⚡ **2026-03-31**：优化缓存策略并提升加载性能
- ⚡ **2026-03-27**：优化缓存策略并提升加载性能
- 🛡️ **2026-03-15**：优化登录会话与安全防护
- 🔧 **2026-03-14**：``` feat(api): 添加SQL注入防护和分页限制
- 🐞 **2026-03-07**：修复若干问题并提升稳定性
- 🔧 **2026-03-04**：``` feat(icon-worker): 更新图标获取API列表
- 🧰 **2026-02-26**：更新文档与部署使用说明
